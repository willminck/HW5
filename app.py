...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/wminckHW5"
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Updated priority of " + item.description)
        return redirect(url_for("all_lists"))
...
def delete(lst):
    form = DeleteButtonForm()
    todo_list = TodoList.query.filter_by(id=lst).first()
    db.session.delete(todo_list)
    db.session.commit()
    flash("Deleted list " + todo_list.title)
    return redirect(url_for("all_lists"))
